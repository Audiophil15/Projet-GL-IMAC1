...
void movingBlock::updatePosition(){
	...
	this->position.x = this->initialPosition.x+(this->finalPosition.x-this->initialPosition.x)*sin(2*M_PI*0.5*SDL_GetTicks()/1000.0);
	this->position.y = this->initialPosition.y+(this->finalPosition.y-this->initialPosition.y)*sin(2*M_PI*0.5*SDL_GetTicks()/1000.0);
